...
def progress_rept(tumor_short, nr_genes, ctr, t0):
	if (ctr%5000)>0: return
	print("\t\t {}: {} out of {}, {} mins".
		format(tumor_short, ctr, nr_genes,"%.1f" % (float(time.time() - t0) / 60)))
	return
...
def cooccurrence(tables, other_args):
	...
	for table in tables:
		...
		print("=" * 20, "\n", table, "donors: ", total_number_of_donors, "donors with mutated %s:" % bg_gene,
			  nr_of_donors_w_bg_gene_mutated)
		...
		outlines = []
		ctr = 0
		nr_genes = len(list(patients_with_muts_in_gene.keys()))
		for gene in patients_with_muts_in_gene.keys():
			...
			progress_rept(tumor_short, nr_genes, ctr, t0)
...
def main():
	...
	outdir = "{}_cooccurrence".format(bg_gene)
