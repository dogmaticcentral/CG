...
def main():
    ...
    for db_name in db_names:
        ...
        for gene_id in genes_seen:
            ...
            #duplicates =  [x for x, y in collections.Counter(samples).items() if y > 1]
            ...
            for i in range (len(samples)):
                #if samples[i] not in duplicates: continue
                start, end = ranges[i].rsplit("-")
                start = int(start)
                end   = int(end)
                print  "  %4d   %5s   %5d   %10d  %10d   %6d " %  (  int(samples[i]), fold_changes[i],  int(num_pbs[i]),  start,  end, end-start)
